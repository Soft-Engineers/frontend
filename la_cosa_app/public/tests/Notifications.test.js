...
describe('Notifications', () => {
    it('Renderiza bien mensajes de turno y jugada', () => {
        ...
        expect(mensajeTurno).toHaveStyle('color: blue; borderBottom: 2px solid black');
    });

    it('Renderiza bien mensajes de infeccion', () => {
        const { getByText } = render(
            <MatchProvider>
                <Notifications messages={['LA COSA TE HA INFECTADO!!']} />
            </MatchProvider>
        );
        const mensajeInfeccion = getByText('LA COSA TE HA INFECTADO!!');
